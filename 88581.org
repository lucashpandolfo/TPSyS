...
#+LATEX_HEADER: \setlength{\parskip}{0}
#+LATEX_HEADER: \parindent=0
#+TITLE: 
#+OPTIONS:  toc:nil
...
#+begin_src octave :exports results :results file
  ...
  factor = 2;
   ...
   #   fft_datos(i,:) = ifft(fft_chirp .* fft_datos(i,:));
   # end
   # toc
  ...
  save("-mat", "data/comprimida-en-rango.mat", "fft_datos_t");
  
  figure(1, 'visible' , 'off');
  
  tic
  imagesc(10*log10(abs(fft_datos_t)));
  toc
...
#+begin_src octave :exports results :results file
  ...
  en_rango = load("data/comprimida-en-rango.mat").fft_datos_t;
  ...
  for i= 1:columns(transformadas)
    transformadas2(:,i) = abs(ifft(fft(transformadas(:,i),2*tam).*fft(filtro,2*tam)))(1:end/2);
  end
  toc
  ...
  figure
  hold on 
  plot(rango,transformadas(:,600));
  plot(rango(n:end), transformadas2(:,600)(n:end),'color', 'red');
  hold off
  axis([rango(1) rango(end)])
  grid()
  
  cambiarEjeX("%0.00fHz", PRF/(2*rango(end)))
  cambiarEjeY("%ddB", 1);
  ...
  figure()
  hold on
  plot(fc1, 'linewidth', 3);
  plot(fc2, 'color', 'red','linewidth', 3);
  plot(bw, 'color', 'g', 'linewidth', 3);
  plot(centroide, 'color', 'm', 'linewidth', 3);
  ...
  save("-mat", "data/frecuencias.mat", "fc1", "fc2", "bw");
...
#+begin_src octave :exports results :results file
  ...
  tic
  for i= 1:columns(transformadas2)
    transformadas3(:,i) = abs(ifft(fft(transformadas2(:,i),2*tam).*fft(filtro,2*tam)))(1:end/2);
  end
  toc
  ...
  hold on 
  plot(rango,transformadas(:,600));
  plot(rango(128:end), transformadas3(:,600)(128:end),'color', 'red');
  hold off
  axis([rango(1) rango(end)])
  grid()
...
#+begin_src octave :exports results :results file
  ...
  duracion = load("data/duraciones.mat").t_chirp;
  
  matriz = load("data/comprimida-en-rango.mat").fft_datos_t;
  
  l_max = rows(matriz())+duracion(end-500)*PRF-1;
  ...
  colormap('gray');
  imagesc(10*log10(abs(comprimida(1:5000,395:end-500))));
...
#+begin_src octave :exports results :results file
  ...
  save("-mat", "data/comprimida-en-azimuth.mat", "azimuth");

  colormap('gray');
  imagesc(azimuth(1:5000,395:end-500));
...
** Opcional 2
   ...
   #+ATTR_LaTeX: width=0.9\columnwidth placement=[H]
#+begin_src octave :exports results :results file
  matriz_0 = load("data/comprimida-en-azimuth.mat").azimuth;
  ...
  imagesc((abs(filtrada))(1:8:end,349:end-500));

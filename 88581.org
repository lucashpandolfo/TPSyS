...
#+begin_src matlab :exports results :results file
  ...
  tic
  for i= 1:columns(transformadas)-64
      temp = abs(ifft(fft(transformadas(:,i),2*tam).*fft(filtro,2*tam)));
    transformadas2(:,i+64) = temp(1:end/2);
  end
  toc
...
** Ejercicio 19
   ...
   $$y[n] = \frac{a_{-7} x[n-7] + a_{-6} x[n-6] + a_{-5} x[n-5] + \cdots + a_{6} x[n+6] + a_{7} x[n+7]}{N}$$

   Donde $a_n$ son coeficientes que forman una ventana de bartlett de
   ancho 15 centrada en cero, y $\displaystyle N = \sum_{n=-7}^{7} a_n$.
...
#+begin_src matlab :exports results :results file
  ...
  tic
  for i= 1:columns(transformadas2)-8
    temp = abs(ifft(fft(transformadas2(:,i),2*tam).*fft(filtro,2*tam)));
    transformadas3(:,i+8) = temp(1:end/2);
  end
  toc
...
#+begin_src matlab :exports none
    ...
    for i=395:columns(matriz)-1200
      ...
      columna = circshift(matriz(:,i),0*round(centroide*rows(matriz)/PRF));
...
#+begin_src matlab :exports results :results file
  ...
  matriz1=matriz;
  tic
  for i = 1:columns(matriz)-4
    matriz1(:,i+4) = ifft(matriz(:,i).*filtro);
  end
  toc
